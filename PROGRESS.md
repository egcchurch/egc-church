...
**Current milestone:** Nav/logo visual redesign (Session 129) shipped on top of the Session 128 work: the real church logo now appears in the nav (centered, transparent over the homepage video hero, solid navy elsewhere/on scroll) and in the footer (previously missing from both). Found and fixed a real, pre-existing, site-wide bug along the way — the nav's `position: sticky` has never actually worked on any page, because `#nav-placeholder`'s box was exactly as tall as the `<nav>` inside it, leaving no room for it to stick past its own height; fixed everywhere with `display: contents` on the placeholder. Per-page custom hero images (admin-uploadable, navy fallback when unset) are scoped as a deferred follow-up, not built yet. Maintenance backlog carried over: installed-PWA rotation still not confirmed on the user's device (Android WebAPK rebuild delay, outside our control) — Phase 3 WhatsApp Stage 2 still pending the church's WhatsApp number
