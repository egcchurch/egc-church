...
| `equipment.manage`   | Full control of the church-wide equipment register on `/members/equipment` — add/edit/retire items, see purchase costs, and manage the equipment-users access list. Ordinary equipment users (who can view the register and run moves) are NOT permission holders — they're uids on the `/equipmentAccess/users` doc, managed from the page itself — see `docs/EQUIPMENT.md` |

**18 keys total.** Easy to extend later if `.manage` ever needs to split into `.create / .edit / .delete`, but start coarse.
